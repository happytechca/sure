...
class TradesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "update with recalculate price adjusts price" do
    patch trade_url(@entry), params: {
      recalculate: "price",
      entry: {
        amount: 209.95,
    ...
    # price = (209.95 - 9.95) / 10 = 20.0
    assert_in_delta 20.0, @entry.trade.price.to_f, 0.001
    assert_in_delta 209.95, @entry.amount.to_f, 0.001
  end

  test "update with recalculate fee adjusts fee" do
    patch trade_url(@entry), params: {
      recalculate: "fee",
      entry: {
        amount: 209.95,
    ...
    # fee = 209.95 - (10 * 20) = 9.95
    assert_in_delta 9.95, @entry.trade.fee.to_f, 0.001
    assert_in_delta 209.95, @entry.amount.to_f, 0.001
  end

  test "update without recalculate includes fee in amount" do
    patch trade_url(@entry), params: {
      entry: {
        currency: "USD",
        nature: "outflow",
